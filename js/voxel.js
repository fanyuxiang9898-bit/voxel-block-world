/**
 * 像素方块世界 - 体素引擎核心模块
 * 包含：方块定义、纹理图集、区块管理、世界生成、Agentic Engine 文字立墙
 */
...
export class World {
  ...
  // Agentic Engine：共享基线，小写 g 保留下降部；细笔画字母使用窄字宽，字间留一格空白。
  // 字模从上到下排列，1 为树叶方块、0 为空气。
  static LETTERS = {
    A: ['01110','10001','10001','11111','10001','10001','10001','00000','00000'],
    g: ['00000','00000','01110','10001','10001','10001','01111','00001','01110'],
    e: ['00000','00000','01110','10001','11111','10000','01111','00000','00000'],
    n: ['00000','00000','11110','10001','10001','10001','10001','00000','00000'],
    t: ['010','010','111','010','010','010','011','000','000'],
    i: ['1','0','1','1','1','1','1','0','0'],
    c: ['00000','00000','01111','10000','10000','10000','01111','00000','00000'],
    E: ['11111','10000','10000','11110','10000','10000','11111','00000','00000'],
  };
  static TEXT_LINES = ['Agentic', 'Engine'];
  static LETTER_SIZE = 9;
  static GAP = 1;
  static LINE_GAP = 2;
  static lineWidth(line) {
    return [...line].reduce((sum, ch) => sum + World.LETTERS[ch][0].length, 0) + (line.length - 1) * World.GAP;
  }
  static TOTAL_W = Math.max(...World.TEXT_LINES.map(line => World.lineWidth(line)));
  static TEXT_HEIGHT = World.TEXT_LINES.length * World.LETTER_SIZE + (World.TEXT_LINES.length - 1) * World.LINE_GAP;
  static TEXT_GROUND_Y = 18;
  static TEXT_FLAT_RADIUS_X = 22;
  static TEXT_FLAT_RADIUS_Z = 10;
  static TEXT_BASE_Y = World.TEXT_GROUND_Y + 1;
  static TEXT_WALL_Z = 0;
  static TEXT_WALL_DEPTH = 1;

  /** 获取文字方块，世界 Y 向上，因此要翻转字模行号。 */
  _getTextBlock(wx, wy, wz) {
    if (Math.abs(wz - World.TEXT_WALL_Z) > Math.floor(World.TEXT_WALL_DEPTH / 2)) return BlockType.AIR;
    const row = World.TEXT_BASE_Y + World.TEXT_HEIGHT - 1 - wy;
    if (row < 0 || row >= World.TEXT_HEIGHT) return BlockType.AIR;
    const lineIndex = Math.floor(row / (World.LETTER_SIZE + World.LINE_GAP));
    const glyphRow = row % (World.LETTER_SIZE + World.LINE_GAP);
    if (glyphRow >= World.LETTER_SIZE) return BlockType.AIR;
    const line = World.TEXT_LINES[lineIndex];
    const width = World.lineWidth(line);
    let x = wx + Math.floor(width / 2);
    if (x < 0 || x >= width) return BlockType.AIR;
    for (const ch of line) {
      const glyph = World.LETTERS[ch];
      const glyphWidth = glyph[0].length;
      if (x < glyphWidth) return glyph[glyphRow][x] === '1' ? BlockType.LEAVES : BlockType.AIR;
      x -= glyphWidth;
      if (x < World.GAP) return BlockType.AIR;
      x -= World.GAP;
    }
    return BlockType.AIR;
  }
      ...
      for (let lz = 0; lz < CHUNK_SIZE; lz++) {
        for (let lx = 0; lx < CHUNK_SIZE; lx++) {
          ...
          for (let y = 0; y < CHUNK_HEIGHT; y++) {
            ...
            else if (y >= World.TEXT_BASE_Y && y < World.TEXT_BASE_Y + World.TEXT_HEIGHT) {
